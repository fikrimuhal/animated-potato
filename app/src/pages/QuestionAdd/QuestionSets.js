...
export default class QuestionSets extends React.Component {
    ...
    shouldComponentUpdate = function (nextProps, nextState) {
        ...
        currentProps = Immutable.fromJS(currentProps, (key, value)=> {
            return value.toOrderedMap();
        });
        nextProps = Immutable.fromJS(nextProps, (key, value)=> {
            return value.toOrderedMap();
        });
    ...
    handleChekboxSetClick = (setId, key)=> {
        let foundKey = this.props.setsOfQuestion.findKey(k => {
            return k == setId;
        });
        var newMap;
                    ...
                    <Row>
                        {
                            this.props.allSet.map((qSet)=> {
                                let foundKey = setsOfQuestion.findKey(k => {
                                    return k == qSet.id;
                                });
                                let checked = foundKey != undefined;
                                return (
                                    <Col lg>
                                        <Checkbox key={qSet.id} value={qSet.id}
                                                  label={qSet.title + "(" + (counter++) + ")"} checked={checked}
