...
export default class StaffList extends React.Component {
    ...
    getOptionCell = (rowData) => {
        return (<div>
            ...
            <FlatButton icon={<GavelIcon/>} onClick={this.makeAdmin(rowData.id)} label={"Make Admin"}
    ...
    unmakeAdmin = id => ()=> {
        var _this = this;
        log("unmakeAdmin row ->", id);
        _this.setState({
            showWaitingPanel: true
        });
        api.UserAPI.unmakeAdmin({
            id: id
        }).then(response=> {
            return response.json()
        }).then(json=> {
            if (json.status == "OK") {
                _this.context.showMessage("Successfully removed admin role.", 2000);
                var rows = _this.state.originalData;
                var index = _.findIndex(rows, q => q.id == id);
