...
export const QuestionCaching = {
    ...
    check: (questionId)=> {
        ...
        if (diff.minute > 5) result = false;
            ...
            var questions = _.filter(cache.data, (q)=> {
...
export const UserResultsCaching = {
    cache: data=> {
        var cacheData = {
            data: data,
            createdTime: Date.now()
        };
        localStorage.setItem("userResultsCache", JSON.stringify(cacheData));
    },
    getAll: ()=>{
        var cacheData = localStorage.getItem("userResultsCache");
        var list = [];
        if (cacheData != null) {
            list = JSON.parse(cacheData).data;
        }
    ...
    check:()=>{
        ...
        var now = Date.now();
        var diff = time.timeDiff(now, cacheTime);
        return diff.minute <= 10;
    },
    clear: ()=> {
        localStorage.removeItem("userResultsCache");
    }
}