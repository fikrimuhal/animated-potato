...
export const getQuestionsBySetName = function getQuestionsBySetName(setName) {
    ...
    var _questions = _.filter(allQuestion, (q)=> {
        return _.includes(q.setList, setName);
    });
    return _questions;
...
export const getUserByEmail = function getUserByEmail(email) {
    var users = getUsers();
    var findIndex = _.findIndex(users, (user)=> {
...
export const getApplicantByUserId = function getApplicantByUserId(userId) {
    var applicants = getApplicants();
    var applicant = _.filter(applicants, (x)=> {
...
export const getApiPromise = (url, method, addToken=true) => data => {
    var headers = {
